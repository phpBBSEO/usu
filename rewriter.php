	...
	*/
	public static function memberlist() {
		...
		if ( @core::$get_vars['mode'] === 'viewprofile' && !@empty(core::$seo_url['user'][core::$get_vars['u']]) ) {
			...
			return;
		} elseif ( @core::$get_vars['mode'] === 'group' && !@empty(core::$seo_url['group'][core::$get_vars['g']]) ) {
			...
			return;
		} elseif (@core::$get_vars['mode'] === 'leaders') {
	...
	*/
	public static function search() {
		...
		if ( $user_id && isset(core::$seo_url['user'][$user_id]) ) {
			...
			return;
		} elseif ( core::$seo_opt['profile_noids'] && !empty(core::$get_vars['author']) ) {
			// Filter default params
			core::filter_get_var(core::$get_filter['search']);
			...
			return;
		} elseif (!empty(core::$get_vars['search_id'])) {
